...
stabMapGeneralised = function(assay_list,
  ...
  for (reference_dataset in names(assay_list)) {
    ...
    if (TRUE) {
      reference_scores = sm(calculatePCA(assay_list[[reference_dataset]], ncomponents = nPC,
                                         scale = FALSE))
      ...
      # reference_pca = prcomp_irlba(t(assay_list[[reference_dataset]]), n = nPC,
      #                              center = FALSE, scale. = FALSE)
      # reference_loadings = reference_pca$rotation
      # rownames(reference_loadings) <- rownames(assay_list[[reference_dataset]])
      # reference_scores = reference_pca$x
      # rownames(reference_scores) <- colnames(assay_list[[reference_dataset]])
      reference_scores = sm(calculatePCA_irlba(assay_list[[reference_dataset]],
                                               nPC = nPC))
    }
    ...
    for (path in all_paths) {
      ...
      while(length(path_current) > 1) {
        ...
        if (path_current[1] == reference_dataset) {
          current_scores = reference_scores
        } else {
      ...
      # experimental: checking for numerical overflow?
      # matList[[length(matList) + 1]] <- t(assay_list[[path_current]])
      # tmp = Reduce("%*%", rev(matList))
      # tmp2 = t(assay_list[[path_current]]) %**% tmp
    ...
    all_embeddings_list[[reference_dataset]] <- embedding
