...
smFISH_gene_symbols = scan(what = "character", file = "../data/smFISH_genes.txt")
table(smFISH_gene_symbols %in% rowData(SCE_thin)[,2]) # none missing
smFISH_genes = rownames(SCE_thin)[rowData(SCE_thin)[,2] %in% smFISH_gene_symbols]
length(smFISH_genes)
```
...
scores_both = getSubsetUncertainty(SCE_thin,
                                  querySCE = SCE[, setdiff(colnames(SCE), colnames(SCE_thin))],
                                  subsetGenes = c(seqFISH_genes, smFISH_genes),
                                  full_sim = full_sim,
                                  plot = TRUE,
                                  plotAdditional = plotAdditional,
                                  jointBatchFactor = joint_sample)
...
ggplot(scores_all_df, aes(x = score, y = score_both)) + 
  geom_point(aes(colour = celltype)) + 
  theme_classic() +
  scale_colour_manual(values = celltype_colours) +
  geom_abline(slope = 1, intercept = 0) +
...
ggplot(scores_all_df, aes(x = interaction(sample, celltype), y = scores_both)) + 
  geom_boxplot(aes(fill = celltype)) + 
  theme_classic() +
  scale_fill_manual(values = celltype_colours) +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
...
ggplot(scores_all_df, aes(x = celltype, y = scores_both)) + 
  geom_boxplot(aes(fill = celltype)) + 
  theme_classic() +
  scale_fill_manual(values = celltype_colours) +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
...
ggplot(scores_all_df, aes(x = interaction(stage, celltype), y = scores_both)) + 
  geom_boxplot(aes(fill = celltype)) + 
  theme_classic() +
  scale_fill_manual(values = celltype_colours) +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
...
ggplot(scores_all_df, aes(x = celltype, y = scores_both - score)) + 
