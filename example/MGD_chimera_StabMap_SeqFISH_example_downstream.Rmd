...
```{r}
g0 = ggplot(StabMap_meta[sample(rownames(StabMap_meta)),],
...
seqFISH_meta_labels <- seqFISH_meta_labels[seqFISH_meta_labels$celltype != "Low quality",]

g_u1 = ggplot(seqFISH_meta[order(abs(seqFISH_meta$chimera_glm_coef)),], 
              aes(x = StabMap_UMAP1, y = StabMap_UMAP2)) +
  geom_point(aes(colour = chimera_glm_coef), size = 0.5) +
  ...
  xlab("StabMap UMAP1") +
  ylab("StabMap UMAP2") +
  geom_text_repel(aes(label = celltype), data = seqFISH_meta_labels,
                  size = 3) + 
  NULL
g_u1
...
g_u1_leg = as_ggplot(get_legend(g_u1 + theme(legend.position = "bottom") +
                                  guides(colour = guide_colourbar(title = "T-/- enrichment\ncoefficient",
                                                                  ...
                                                                  ncol = 5,
                                                                  override.aes = list(size = 6, alpha = 1)))))
ggsave(g_u1_leg, file = paste0("../../Figures/raw/chimera_seqFISH_stabmap_chimera_coef_leg.pdf"), height = 3, width = 4)
```
...
# perform differential expression according to the AP value
library(limma) 
mm1 = model.matrix(~ AP + embryo, data = seqFISH_meta_spl[cells_spl_spatial,])
fit1 = lmFit(spl_imp, mm1)
fit1 <- eBayes(fit1)
tt1 = topTable(fit1,coef="AP", sort.by = "none", number = Inf)
volcanoplot(fit1, coef = "AP", highlight = 50, names = rownames(tt1))
tt1$gene = rownames(tt1)
g_de_ap = ggplot(tt1, aes(x = logFC, y = -log10(P.Value))) + 
  geom_vline(xintercept = 0) +
  geom_point(colour = "grey") + 
  geom_text_repel(aes(label = gene), data = subset(tt1, rank(P.Value) < 30), size = 7, fontface = "italic") +
  theme_classic() +
  ggtitle("Linear effect test") +
  ...
  NULL
g_de_ap
ggsave(g_de_ap, file = "../../Figures/raw/chimera_seqFISH_stabmap_AP_splanchnicmesoderm_de_ap.pdf",
       height = 9, width = 8)

mm2 = model.matrix(~ AP + I(AP^2) + embryo, data = seqFISH_meta_spl[cells_spl_spatial,])
fit2 = lmFit(spl_imp, mm2)
fit2 <- eBayes(fit2)
tt2 = topTable(fit2,coef="AP", sort.by = "none", number = Inf)
volcanoplot(fit2, coef = "I(AP^2)", highlight = 50, names = rownames(tt2))
tt2$gene = rownames(tt2)
g_de_ap2 = ggplot(tt2, aes(x = logFC, y = -log10(P.Value))) + 
  geom_vline(xintercept = 0) +
  geom_point(colour = "grey") + 
  geom_text_repel(aes(label = gene), data = subset(tt2, rank(P.Value) < 20), size = 7, fontface = "italic") +
  theme_classic() +
  theme(axis.title = element_text(size = 15)) +
  theme(plot.title = element_text(size = 20, hjust = 0.5)) +
  xlab("Log Fold Change") +
  ylab("") +
  ggtitle("Quadratic effect test") +
  NULL
g_de_ap2
ggsave(g_de_ap2, file = "../../Figures/raw/chimera_seqFISH_stabmap_AP_splanchnicmesoderm_de_ap2.pdf",
       height = 9, width = 8)

seqFISH_meta_spl_expr = seqFISH_meta_spl[cells_spl_spatial,]
expr_genes = sort(c("Wnt2", "Tbx1", "Ccnd2", "Foxf1", "Apoe"))
seqFISH_meta_spl_expr[,expr_genes] <- t(spl_imp[expr_genes,])
expr_genes_cols = setNames(nord(n = length(expr_genes), palette = "red_mountain"), expr_genes)
seqFISH_meta_spl_expr_melt = reshape::melt.data.frame(seqFISH_meta_spl_expr, "AP", expr_genes)
g_test = ggplot(seqFISH_meta_spl_expr_melt, aes(x = AP, y = value)) +
